...
ZipMethods = [('DNSLogzip', 'dlz.gz'), ('gzip', 'gz'), ('bzip2', 'bz2'), ('LZMA', '7z')]
...
UnzipCMDs = [
            'rm -rf /media/ramdisk/{exprimentName}.{datasetName}.{resultName}.txt && time -p ( gzip -c -d /media/ramdisk/{exprimentName}.{datasetName}.{resultName}.txt.{suffix} > /media/ramdisk/{exprimentName}.{datasetName}.{resultName}.txt ; DNSLogzip -D < /media/ramdisk/{exprimentName}.{datasetName}.{resultName}.txt > /dev/null ) && rm -rf /media/ramdisk/{exprimentName}.{datasetName}.{resultName}.txt',
            
            'time -p ( gzip -c -d /media/ramdisk/{exprimentName}.{datasetName}.{resultName}.txt.{suffix} > /dev/null )',
            
            'time -p ( bzip2 -d > /dev/null < /media/ramdisk/{exprimentName}.{datasetName}.{resultName}.txt.{suffix} )',
            
            'time -p ( 7za x -so /media/ramdisk/{exprimentName}.{datasetName}.{resultName}.txt.{suffix} > /dev/null )'
            ]
    ...
    def DumpResult(self):
        ...
        with open(self.resultFilePath, 'w+') as f:
            ...
            for k in range(len(ZipMethods)):
                f.write(',{0}_ratio,{0}_speed,{0}_dspeed'.format(ZipMethods[k][0]))
            f.write('\n')
            
            f.write('DatasetName')        
            for k in range(len(ZipMethods)):
                f.write(',{0},{0},{0}'.format(ZipMethods[k][0]))
            f.write('\n')
