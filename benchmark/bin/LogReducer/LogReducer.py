...
def add_argument(parse):
    ...
    parse.add_argument("--CompressingMode", "-C", default="7z", choices=["7z", "gz","bz"], help="The following compression formats are supported(7z,gz,bz)")
...
def procFiles(typename, fileBeginNo, fileEndNo, now_input, now_output, now_temp, type_template):
    ...
    for i in range(fileBeginNo, fileEndNo + 1):
        filename = "{}".format(i)
        if args.CompressingMode == "7z":
            zip_path = str(now_output) + str(filename) + ".7z"
            compression_order = "7za a " + zip_path + " " + now_temp + filename + "/*" + " -m0=LZMA"
        elif args.CompressingMode == "gz":
            zip_path = str(now_output) + str(filename) + ".gz"
            compression_order = "tar -cvzf " + zip_path + " " + now_temp + filename 
        elif args.CompressingMode == "bz":
            zip_path = str(now_output) + str(filename) + ".bz2"
            compression_order = "tar -cvjf " + zip_path + " " + now_temp + filename 
        print(compression_order)
